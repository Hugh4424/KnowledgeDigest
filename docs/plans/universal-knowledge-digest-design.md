...
**实现状态（2026-07-28）**：Phase 0 / 1 / 2.5 已落地；Phase 3 已完成 agentmemory + 公司 Confluence 知识库双库隔离验收。agentmemory 仅通过官方 REST 和现有 `AgentMemoryStore` 验收，不接入正式 `pipeline.py`、`cli.py` 或生产入口；Phase 4 保留 embedding 接入和阈值标定目标。本文是设计原稿，归档于 `specs/archive/kd-phase0-2.5-plans/`。
...
2. 同一套技能以公司 Confluence 知识库为正式文件目标；agentmemory 作为独立的 REST 记忆验收目标，不进入正式管线，文件目标的差异收敛到一份轻量的 KB 结构约定描述（不是代码接口层）。
...
- 处理：临时文件 + fsync → 原子 rename，直接写入公司 `kb_dir`。正式管线不声明 agentmemory 索引同步；**写回失败直接报错退出（let it crash）**，不做持久重试队列、不做告警——手动触发场景下重跑技能本身就是重试。
- 输出：落盘的公司知识条目；agentmemory 仅在独立验收中通过官方 REST 写入和读取。
...
核心管线（S1-S6）不依赖代码接口层，而是依赖一份**公司 KB 结构约定描述文件**（YAML/md 均可）。agentmemory 不作为正式文件目标，只在 Phase 3 隔离验收中通过官方 REST 使用。

KB 结构约定描述文件包含三部分：

| 部分 | 内容 | agentmemory 验收 | 公司知识库示例 |
|---|---|---|---|
| 字段模型 | 该库 md 文件的 frontmatter 字段清单，标注哪些必填 | REST memory provenance 字段 | 13 字段 YAML frontmatter（tier/trust/source_status 等）|
| 目录组织规则 | 新增/更新文件放在哪、命名规则、是否分模块目录 | curated/ 目录 + viking:// URI 映射 | 模块手册体系，H2 自由正文分节 |
| 强制字段 | 本设计要求的字段（why/版本历史，见第5节）在该库如何落到具体字段名 | 映射到 REST provenance 字段 | 映射到 13 字段里哪个字段 |

关键设计原则：
- **没有代码层适配接口**：不再定义 `SourceReader`/`KnowledgeStore`/`IndexSync` 三个抽象类。核心管线直接读写文件系统，行为差异只由"读哪个目录、按什么字段模型写"这份约定描述文件决定。
- **agentmemory 不参与正式索引同步**：Phase 3 只在全新临时现场启动 disposable agentmemory，通过官方 REST 验证写入、搜索、读回、replay 和 provenance；正式管线不调用 agentmemory。
- **字段模型可扩展、不可裁剪**：字段集合由 KB 结构约定描述文件声明，核心管线不假设固定字段数量或名称。
- **embedding/LLM 后端独立配置**：公司知识库和后续 agentmemory 能力各自在自己的配置文件里声明 provider，不做跨库统一路由；Phase 3 默认 `--no-llm`，不做 embedding 或阈值调整。
...
- 验收：构造型 fixture + `tests/acceptance/test_phase0_digest.py`；历史设计中的「OpenViking 真库人工抽查」不属于当前目标，也不作为本项目后续 Phase 3 验收项。
...
### Phase 3（agentmemory + 公司 Confluence 知识库双库验收）
- 从固定公司 Confluence 原始目录复制 3 个新输入，其余 Markdown 作为隔离 company-kb 已有页面，生成结构描述、来源索引和 sha256 证据。
- 使用现有 CLI 完成隔离 `--no-llm` dry-run 和正式运行，确认 company-kb 输出全部 `success`、3/3 新输入处理并保留 provenance。
- 在全新 `/tmp/kd-phase3-agentmemory-run.*` 现场启动 disposable agentmemory，使用独立 `HOME`、state/stream 路径和端口，通过官方 REST 与现有 `AgentMemoryStore` 完成写入、搜索、读回、原样 replay、删除后的 provenance 反向验证红（非 0）→恢复后绿（0）。
- 本阶段不接入正式 `pipeline.py`、`cli.py` 或生产入口。OpenViking 仅保留为历史调研背景，不是当前目标库。

### Phase 4（embedding 接入和阈值标定）
- 接入 embedding，并提供阈值标定工具（参照历史 ovmc 标定协议：标注金标准簇 → 检验特征分离度 → 生成阈值建议），供目标语料自行标定。
...
- **适配层从代码接口降级为约定描述文件**：公司知识库本质是"文件夹+md"，正式管线只需一份 KB 结构约定描述（字段模型+目录规则+强制字段）；agentmemory 使用独立 REST 验收路径。
