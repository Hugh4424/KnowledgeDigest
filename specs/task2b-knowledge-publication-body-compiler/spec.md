...
## 14. 快捷范围调整（2026-08-12）

本次只修复已存在的上游事实投影缺口，不新增产品需求：Task 1 TopicIndex 必须保留真实 source declaration 中已显式声明且可由 source URI、content fingerprint 和 locator 回查的 `page_type`；Task 2-B 消费通过入口对账的新快照后重跑既有 T013/T014。旧快照保留为审计基线；新快照或语义运行任一失败都保持 `incomplete/not_released`。不改变固定 section、三类 page type、机器阈值、provider、Task 2-C/Task 3 边界，也不允许 Task 2-B overlay 或标题/正文猜测。来源：当前 `decision-log.md` C-002；影响 `FR-SEM-001/003`、`AC-01/09/10/11`。

## 15. 快捷路径当前结果（2026-08-12）

- The current Task 1 snapshot is authoritative for this rerun; the old snapshot remains an immutable audit baseline. Explicit page types are source-declared facts with URI, content fingerprint, locator and topic identity bindings.
- The new real semantic T013 completed with qwen3.6 and environment-only credentials, but the validator returned `machine_exit_passed=false`: one machine-passing `module_or_capability` concept, no `product_overview` or `procedure_or_rule`, and fewer than six concepts.
- Task 1, consumer, and full deterministic regressions are green (`49 passed`; `262 passed, 2 skipped`; `516 passed, 3 skipped`; `git diff --check` passed). These are not semantic release evidence.
- The contract therefore remains fail-closed: `incomplete/not_released`, no early Task 2-C, and no Git delivery as a completed Task 2-B result. The deferred handoff is a fresh valid semantic run plus current ordinary-stage evidence after the provider/faithfulness gap is actually resolved.
