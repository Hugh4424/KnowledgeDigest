...
- Focused checks: `uv run --frozen pytest tests/acceptance/test_task1_topic_axis.py -q` → 17 passed; `uv run --frozen pytest tests/acceptance/test_task2_batch_recovery.py tests/acceptance/test_task2_publication.py tests/archive/legacy-reader-pipeline/publication_contract_legacy.py -q` → 54 passed; `python -m py_compile src/knowledge_digest/topic_axis.py src/knowledge_digest/kb_structure.py src/knowledge_digest/pipeline.py src/knowledge_digest/batch_run.py src/knowledge_digest/identity.py` → passed; `git diff --check` → passed.
