...
- 内容信号：`generated`、`digest_machine_pass`、`verified`、`stale_after` 按 Task 2-A/PRD 合同分开；内容变化使旧验证事件失效。
- 增量更新：section 只有在依赖集合、版本、结构关系和归因都可证明未变时才能复用；影响关系不确定时扩大到整页重编。

### 成功边界

- 至少 6 个 machine-passing concept。
- 三类 page type 各至少 1 个；覆盖 Task 1 inventory 中实际存在的长文、表格/图片、双语、多源类别；不存在的类别必须有 machine fixture 或排除理由。
...
- 必需证据缺失、归因不闭合、版本冲突、事实保真失败、provider 失败、截断 JSON、fallback、golden-negative 未失败或近重复命中：页为 `degraded`，不进正式导航。
...
- 当前 contract revision 预算：`0/1`。改变 section 集合、模板、必需/可选字段或 page type 映射消耗唯一一次修订；只修复既有合同的行为 bug 不消耗。
...
- decision: 使用冻结的 12–20 篇样本和其 source/type/answerability 覆盖记录；抽样蕴含记录固定 seed、样本数、判定阈值、检测器/模型和失败项；Task 2-B 只做机器诊断；body/section contract revision 当前消耗 `0/1`，只有改变 section 集合、模板、字段或 page type 映射才消耗。
...
- 质量事实：当前有 PRD/Task 2-A 合同、三轮 Talk、Grill、direction review `pass` 和修订后 detail review `pass`；修订前的两项 major 与四项 minor 已修正/处置，修订后保留一份有效独立 review 结果和一份 `OUTPUT_INVALID` provider 事实。任何这些事实都不等于代码已实现或语义 exit 已通过。
...
- none。Task 2-A 的 Reader Bundle contract 继续有效；本任务只在 PRD 允许的一次正文/section contract revision 范围内新增正文编译决定。

## 文档结果

- CONTEXT.md：no-change；现有术语已覆盖 Claim、fragment_locator、Reader/Audit、page status 和 delivery status；“影响闭包”是本任务实现术语，不改全局词典。
- ADR：created；`docs/adr/0005-task2b-controlled-section-recompile.md` 记录 section 增量更新和整页保守兜底；状态为 proposed，最终接受绑定仍由本阶段决定。
- ADR criteria：hard to reverse=是；surprising without context=是；genuine trade-off=是；三项均成立，因此已创建 ADR。
- 术语/ADR 冲突及处理：detail review 指出旧记录矛盾，已改为 ADR created；Task 2-A 的结构合同与本任务的正文更新策略正交，旧 Reader/Audit 分离 ADR 继续适用。
...
## 继续中的 make-decision（2026-08-12）

本节是对当前 Task 2-B 未闭合事实的 append-only 续接，不改写前文已经接受的正文编译决定；当前续接尚未最终确认，也不产生实现授权。

### C-001 当前用户回复

- source_type/reference/exact_excerpt: user_talk / current continuation / `A`（对应上一张 Talk 卡的“先修复 Task1/入口证据，再回到 Task 2-B 重跑出口”）。
- current_direction: 先修复上游 Task 1/Task 2 entry evidence，再重跑 Task 2-B 真实语义出口；Task 2-C 继续延期到 Task 2-B 真正闭合。
- approval_binding: pending；这是当前 Talk 的实际方向选择，不是最终 decision card confirmation。
- consequence_and_risk: 上游证据范围会变化，必须保留旧快照、生成新 hash 并重新核验；不能把 Task 2-B 的失败变成放宽门禁或临时 fixture 通过。

### C-002 快捷路径授权（2026-08-12）

- source_type/reference/exact_excerpt: user_talk / current continuation / `好的，那就按这个快捷路径继续`。
- selected_path: 同一 TaskHandle 内做受影响材料的最小范围调整；不再重复完整 WorkflowHub 阶段链，也不调用已移除的公开 `scope_revision` review kind。
- affected_scope: Task 1 上游 TopicIndex/entry evidence 的显式 page-type 投影与入口对账；Task 2-B 只消费新快照并重跑 T013/T014/当前普通阶段检查。
- source_of_truth: Task 1 仍拥有 TopicIndex；page type 只接受真实 source declaration 的显式值和 source URI/content fingerprint/locator 绑定；Task 2-B 不新增 overlay、不从标题/正文猜测。
- acceptance_boundary: 新快照必须保留旧快照作为审计基线，并对真实 89 条来源、主题身份、page type 和证据引用重新哈希对账；Task 2-B 仍要求至少 6 个 machine-passing concept、三类 page type 各至少 1 个、实际 inventory 类别覆盖、真实 qwen3.6 语义运行；失败保持 `incomplete/not_released`。
- non_goals: 不改固定 section/page-type/body contract，不降低阈值，不换 provider，不把 Task 2-C 人工门、全量 89 篇或 released 提前完成，不用 fixture/overlay 冒充真实来源。
- rationale/plain_language: 现在卡点是上游 TopicIndex 没把已有的真实 page-type 事实带给 Task 2-B；先补这条证据链，才有资格重跑正文机器门。快捷只减少重复流程，不减少验收门。
- approval_binding: accepted by current user reply; final delivery status remains pending until evidence and Git delivery are actually complete。

### F-003 当前事实核查与研究结论

- source_type/reference: local_research / Task 1 frozen evidence、Task 2 entry manifest、Task 2-B current semantic run、current `pipeline.py`。
- facts_and_constraints:
  - frozen TopicIndex 有 54 个主题，31 个 `published`、23 个 `degraded`；54 条都是 `knowledge_type=products`，没有 `page_type` 或 `digest_page_type` 字段。
  - 31 个已发布主题有 module/object_intent，可按现有确定性规则进入 `module_or_capability`；当前没有可证明的 `product_overview`，也没有可证明的 `procedure_or_rule`。
  - Task 2 entry coverage 明确把 page-type coverage 延期到 Task 2-A/Task 2-B，且没有注册 machine fixture；因此不能把当前入口 manifest 当成三类页面已覆盖。
  - `pipeline.py` 已明确禁止从标题或正文猜 `procedure_or_rule`；Task 1 的 TopicIndex/TopicPlan 才是主题身份和映射事实的上游权威。
  - 当前真实 T013 已完成 provider 调用，但只有 1 个 machine-passing concept，validator 为 `valid=false`、`machine_exit_passed=false`、`reader_eligible=false`；失败原因包含概念数不足及缺少 product overview/procedure。
- research_disposition: 外部调研跳过；这是本地冻结产物、代码契约和当前运行证据决定的问题，外部资料不会改变事实边界。独立根因核查已完成，结论与上述事实一致。
- status: current direction remains `in_progress` / `not_released`；不存在安全的纯 Task 2-B 内容修补可以替代上游证据修复。

### C-003 快捷路径执行结果（2026-08-12）

- selected path executed: Task 1 produced a new current snapshot from the real 89-source corpus; Task 2-B consumed that snapshot through an isolated frozen 20-example/30-source semantic run. The old Task 1 snapshot remains unchanged as the audit baseline.
- Task 1 current snapshot: 54 topics, 31 `published`, 23 `degraded`; explicit page types are bound to source URI, content fingerprint and line-1 locator for `product_overview`, `module_or_capability`, and `procedure_or_rule`. Snapshot receipt hashes are recorded in `quality/evidence/task2-entry/task1-receipt-reconciliation-current-20260812.v1.json`.
- T013: run_id=`run-6827d359b90d467c87d3d955d4a6673d`; evidence=`docs/archive/apply/evidence/T013.semantic-run-task1-repair-20260812.json`; evidence sha256=`dd3e746c44edab76500259a61abbca3f97f8a2dcec83ebbbf5356e01d5afd1ce`; `run_status=completed`; `execution_mode=real_semantic`; provider=`qwen3.6` at the approved endpoint; credential=`environment-only`; calls=`15/15`.
- T013 machine result: `1` machine-passing concept, `module_or_capability` only, `evidence_backtrace=168`, complete sections=`1`; validator=`valid=false`, `machine_exit_passed=false`, `reader_eligible=false`; missing `product_overview` and `procedure_or_rule`, concept count below `6`; delivery remains `not_released`.
- T014: Task 1 focused=`49 passed`; consumer regression=`262 passed, 2 skipped`; full regression=`516 passed, 3 skipped`; `git diff --check` passed. These prove deterministic compatibility only and do not override the semantic gate.
- choice/rationale: keep the repaired upstream projection because it is traceable and is now consumed by T013; keep fail-closed because the provider/faithfulness output still does not meet the existing semantic contract. Do not relax gates, infer page type, create an overlay, or use fixtures as real coverage.
- deferred handoff: Task 2-B remains `incomplete/not_released`; no commit/merge/push/cleanup as a completed delivery; Task 2-C remains deferred until a fresh T013/T014 machine exit genuinely passes and current-stage evidence is current.

### T-004 当前续接的下一决策轴（等待用户回复）

- question: 修复上游时，是否允许 Task 1 重新产出带可追溯 page-type 证据的当前 TopicIndex/entry snapshot，还是只补一份不改 TopicIndex 的登记？
- options_and_risks:
  - A（推荐）：重跑/修正 Task 1 当前输出，保留旧快照，并为真实 product overview、module/capability、procedure/rule 候选绑定 source URI、内容 hash、locator、topic identity；procedure 另做确定性 exceptions audit。后果是要重新核验 Task 1，但事实源只有一套；风险是可能重新打开 Task 1 的边界问题。
  - B：只补 entry manifest/登记，不改 TopicIndex。后果是改动较小；风险是 page type 仍有两套事实源，Task 2-B 可能继续被合法门禁拦住。
  - C：在 Task 2-B 增加 overlay 直接映射。后果是看起来最快；风险是绕过 Task 1 上游权威，形成平行事实源，不接受。
- queue_change: F-003 已关闭“是否真的存在上游缺口”事实轴；T-004 等待用户选择后，才进入 direction advice review；不提前运行 review，不进入 build-spec。

- user_reply: `A`；用户明确选择“重跑/修正 Task 1 当前输出，保留旧快照，并用可追溯来源证据补齐 page type”，同意承担重新核验 Task 1 的后果。
- selected_option: A；T-004 已收敛，进入 direction advice review；当前仍不是最终 decision card confirmation。

### F-004 direction advice review（2026-08-12）

- review_status: available；实际有效审查为 `opencode/v4flash`；`codex/luna` 因与宿主同源被记录为 `SAME_SOURCE`，不伪装成第二份建议。
- review_scope: 审查“修复 Task 1/entry evidence 后再重跑 Task 2-B”的方向是否有可追溯事实、明确上下游边界、可判定的 Task 2-B/Task 2-C 交接、明确非目标、procedure exceptions audit 失败语义和不可修复时的失败语义。
- findings:
  - 事实必须逐项绑定 TopicIndex、entry manifest、current semantic run、代码契约或用户回复的稳定来源；不能只在 review packet 中写一行无引用的断言。
  - Task 1/entry 修复属于上游前置，不得无边界地吞进 Task 2-B；需要写清 owner、完成条件和“完成后才允许重跑”。
  - 必须写清新旧快照关系：旧快照保留为审计基线；通过入口复核的新快照才是本次重跑及后续 Task 2-C 交接的权威。
  - Task 2-B 不能要求 54 个主题全部非 degraded；本任务的机器出口仍是至少 6 个 machine-passing concept、三类 page type 各至少 1 个、实际 inventory 特征覆盖，且不可用临时 fixture 或放宽门禁替代真实证据。
  - page-type coverage 对本次 Task 2-B 语义出口不是延期项；machine fixture 只保留为 PRD 明确允许的兜底，不作为本次真实来源修复的目标。
  - procedure exceptions audit 不能绕过 source URI、content hash、locator、topic identity 绑定；无法绑定的例外只能记录为 degraded，不能进入 published。
  - 上游证据无法修复时必须停止重跑，保持 `not_released`，保留旧快照和失败审计。
- disposition: 以上方向性 finding 进入 Talk Round 3，等待用户确认边界；在确认前不修改 Task 1/entry，不运行 Task 2-B，不进入 build-spec。
- evidence_ref: WorkflowHub direction review attempt/result/report 已保存在当前 TaskHandle 的 `quality/reviews/`；provider 原始私有路径不进入本日志。

### T-005 Talk Round 3（等待用户回复）

- question: 上游修复和 Task 2-B 重跑的边界，是否按下面的方式锁定？
- recommendation/plain_language: 推荐；先把上游证据修好并单独验收，Task 2-B 只消费验收后的新快照，不把两个阶段揉成一团。
- proposed_direction:
  - Task 1/entry repair 是上游前置工作；完成条件是生成新的带 hash 的 TopicIndex/entry snapshot，并逐项绑定真实来源、content hash、locator、topic identity 和 page type，通过入口对账。
  - 旧快照只保留作审计基线；新快照通过入口对账后，才成为本次 Task 2-B 重跑和后续 Task 2-C 交接的权威。
  - Task 2-B 通过条件仍是原合同：至少 6 个 machine-passing concept、三类 page type 各至少 1 个、实际 inventory 特征覆盖；不要求 54 个主题全部非 degraded。
  - page-type coverage 必须为本次语义出口补齐；不做全量 89 篇、Task 2-C 人工读者门、正式 released，也不新增平行 overlay。machine fixture 不是本次目标；只有 PRD 明确兜底且真实来源无法提供时才另行决策。
  - procedure exceptions audit 只做审计，不放宽发布门；不能绑定完整证据的例外保持 degraded。
  - 上游修复或后续语义重跑任一步失败，都保留旧快照和失败证据，交付保持 `not_released`，不启动 Task 2-C。
- options:
  - A（推荐）：接受上述边界，先单独完成上游入口修复和验收，再重跑 Task 2-B。
  - B：把上游修复算进 Task 2-B 一起做。后果是阶段边界变模糊、验收无法知道失败来自哪里；风险是后续可能把未验收的上游结果当成 Task 2-B 成功。
  - C：允许先用不完整入口直接重跑，缺 page type 的部分继续 degraded。后果是可能得到局部结果；风险是不能满足三类 page type 的语义出口，不能进入 Task 2-C。
- queue_change: T-005 等待用户选择；收到后才进入 Grill。当前不执行实现、不重跑 provider。

### C-004 真实语义出口闭合事实（2026-08-12）

- original_requirement: Task 2-B must turn the three typed topic pages into readable, traceable pages; every release decision must keep source/Claim lineage, fail closed on bad output, and keep Task 2-C human review separate.
- facts: the current Task 1 snapshot contains 89 real sources, 54 topics, 31 published and 23 degraded; explicit source-declared page types are bound to URI, content fingerprint, locator and topic identity. The v9 real run used the current snapshot, frozen 20-example/30-source input, qwen3.6 at the approved endpoint and environment-only credentials.
- result: T013 run_id=`run-519d5c93591e45faab8e3ef56601a3f1`, evidence=`docs/archive/apply/evidence/T013.semantic-run-task2b-provider-repair-v9-20260812.json`, evidence sha256=`c38aad3185bd534ee988766d55fc26ee68d5f8b2688f8e00ddb72d23dbbd17e4`; 12 concepts passed the machine gate, with page types `module_or_capability=10`, `product_overview=1`, `procedure_or_rule=1`; canonical validator returned `machine_exit_passed=true`.
- choice: keep the existing thresholds, three page types, source-derived Task 1 authority, provider/model, fail-closed body gate and `delivery_status=not_released`. The implementation fixes only provider prompt/repair diagnostics, list-marker normalization, and deterministic shortening of confirmed Evidence-dump sections; it does not relax the source block gate or invent facts.
- rationale/plain_language: the earlier failure was not fixed by pretending four copied source sentences were readable. The provider gets a bounded repair chance; if it still returns an unambiguous raw evidence stack, the compiler keeps the longest safe prefix in Reader and leaves omitted Claims in Evidence. Ambiguous rewrites still degrade. This makes the machine result pass without erasing lineage or weakening the gate.
- T014: `docs/archive/apply/evidence/T014.final-regression-task2b-provider-repair-v9-20260812.txt` records Task 1=`49 passed`, consumers=`264 passed, 2 skipped`, full=`518 passed, 3 skipped`, and `git diff --check` pass; routing=`docs/archive/apply/evidence/T014.test-routing-task2b-provider-repair-v9-20260812.json`.
- deferred handoff: Task 2-B machine work is complete but delivery remains `not_released`; Task 2-C must start from `make-decision` and handle human reader quality, not be inferred from the machine gate. Formal Git commit/merge/push/cleanup is not claimed in this record.
