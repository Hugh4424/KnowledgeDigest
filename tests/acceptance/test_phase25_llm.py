...
Every test replaces ``urllib.request.urlopen`` so no real network request is
made. The provider contract, the fail-closed error paths, and the interaction
between LLM output and the existing ``_validate_candidate`` hard gates are all
exercised against stub responses.
"""
...
from knowledge_digest.draft import draft, resolve_generator
...
def test_llm_forging_claim_raw_id_is_rejected(
    ...
    forged_coverage = dict(coverage[0], raw_id="raw-999")
    result = _llm_draft(
        monkeypatch,
        tmp_path,
        {
